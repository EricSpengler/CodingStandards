...
The standards in `standards/` are written to be shared across projects. This file holds everything true of **this** project and not necessarily of any other: the values behind the `project`- and `product`-scoped constants in Section 0, and the rules that exist because of this product's requirements rather than because of good C++ practice.
...
**This tool does not process PHI, PII, or other personally-regulated data.** CUI and export control, per 4.1, are the only sensitivity classifications that apply to this codebase and the data it handles.
...
The following was rule 2.5 in the standard. It is a product licensing and distribution requirement, not a coding standard, so it lives here. The text is unchanged from the version that was in `standards/02-versioning.md`; references to other rules still point at the standard.
...
- **C-45** — the export-control point of contact named in 4.1 is currently unnamed.
