...
Numbered `0` so it sorts ahead of P1 without disturbing any existing section number or cross-reference.
...
3. **A project-scoped value written out in a rule without citing its constant id** — the two-sources-of-truth problem rule C2.6 prohibits for documentation. This check is what found C-14, where the commit scope list was written out in both P1.3.1 and the project profile, and C-31.

What it deliberately does *not* check is whether a stated value is *correct* — that `72 characters` in P1.3.1 still matches C-15. Doing that reliably would need a machine-readable probe per constant, which is more registry bookkeeping than the drift risk justifies. The **Used in** column is the manual answer: change a value, visit the rules it names.
...
|---|---|---|---|---|
| C-01 | Issue tracker | Jira | `org` | P1.2.1, P1.2.2, P1.2.3, P1.3.1, P1.3.3, P1.3.4, P1.4.1, P1.7.3, P2.1.1, P2.4.1 |
| C-02 | Ticket key format | `JIRA-XXX` | `org` | same as C-01 |
| C-03 | Repository host | GitLab | `org` | P1.1.1, P1.2.1, P1.3.1, P1.4.1, P1.4.2, P1.4.3, P1.5.1, P1.7.1, P1.7.3, P1.8.1 |
| C-04 | MR template path | `.gitlab/merge_request_templates/` | `org` | P1.7.3 |
...
| ID | Constant | Current value | Scope | Used in |
|---|---|---|---|---|
| C-13 | Commit type vocabulary | feat, fix, style, chore, docs, refactor, test, perf, build, ci, revert | `org` | P1.2.2, P1.3.1 |
| C-14 | Commit scope vocabulary | *see project profile* | `project` | P1.3.1 |
| C-15 | Commit subject cap | 72 characters | `org` | P1.3.1 |
| C-16 | Commit body wrap | 100 characters | `org` | P1.3.1 |
| C-17 | Version scheme | `0.MINOR.PATCH`, MAJOR pinned pre-1.0 | `org` | P2.1.1 |
| C-18 | Changelog tool and config | git-cliff, `cliff.toml` | `org` | P2.4.2, App. D |
| C-19 | Changelog file | `CHANGELOG.md` | `org` | P2.4.1 |
...
| C-27 | Supported platforms | *see project profile* | `project` | P1.8.1 |

## Code layout

| ID | Constant | Current value | Scope | Used in |
|---|---|---|---|---|
| C-28 | Top-level source directories | *see project profile* | `project` | C1.2, C1.4, C1.11, C1.19, C2.4, App. A |
| C-29 | Source file extensions | `.h` / `.cpp` | `org` | C1.3, C1.17, C1.19, P3.1 |
| C-30 | Test fixture directory | *see project profile* | `project` | P1.6.1, P1.6.2, P1.6.3 |
| C-31 | Namespace documentation file | *see project profile* | `project` | C2.4, App. A |
| C-32 | Third-party include prefixes | *see project profile* | `project` | C3.5.8, App. B |

## Style limits

| ID | Constant | Current value | Scope | Used in |
|---|---|---|---|---|
| C-33 | Indent width | 4 spaces, no tabs | `org` | C3.5.2, App. B |
| C-34 | Column limit | 100 | `org` | C3.5.3, App. B |
| C-35 | Brace style | Allman | `org` | C3.5.1, App. B |
| C-36 | Max function length | 60 lines | `org` | C3.2.1, App. C |
| C-37 | Complexity ceiling | 10 (see D-03 for the metric) | `org` | C3.2.2, App. C |
| C-38 | Max nesting depth | 3 | `org` | C3.2.3, App. C |
| C-39 | Max function parameters | 5 | `org` | C3.2.4, App. C |
| C-40 | Max file length | 1000 lines | `org` | C3.2.5 |
...
| C-42 | Doxygen comment style | Javadoc `/** ... */` | `org` | C2.1, App. A |

## Classification and export control

| ID | Constant | Current value | Scope | Used in |
|---|---|---|---|---|
| C-43 | Default classification marking | `UNCLASSIFIED` | `org` | C1.3, P3.1, C2.4 |
| C-44 | Default export-control statement | "not subject to export control regulations" | `org` | P3.1 |
| C-45 | Export-control point of contact | *see project profile — still unnamed* | `project` | P3.1 |
...
| C-47 | Build expiry mechanism | *moved* | 2.5 removed; P2.2 generalized | **done** — rule text verbatim in `project/PROJECT_PROFILE.md`. Section number 2.5 left unused rather than reassigned |
| C-48 | Data sensitivity determination | *moved* | P3.2 rewritten | **done** — P3.2 now requires every project to record a determination; this project's answer is in `project/PROJECT_PROFILE.md` |
| C-49 | Running example types | `RecordReader`, `RecordBatch`, `ReadError`, `FileHandle` | §P1, 3, 5, 6 | **done** — domain-neutral throughout. `utf8decoder` retained in C1.1 and C1.3, which need a version-like token to demonstrate the fusion rule at all |
