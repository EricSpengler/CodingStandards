...
Grouped by domain: **Process** (any language) versus **Language — C++** (the only language standard written so far; Python, Java, C#, and MATLAB will get their own tables here once work on them starts, per R-07). Landing places are indicative, not committed. What is settled is the domain each topic belongs to, since that decides which folder it lands in and therefore its prefix.
...
| T-25 | CI/CD pipelines — pipeline stages, required checks, what gates a merge | P9 (CI/CD) | open |
...
Nothing here can be resolved by drafting. Each blocks or reshapes the work named in "Lands in". Open decisions are grouped by domain; already-decided ones are kept together below, un-split, as historical record.
...
From the standards review. Open findings are grouped by domain; done rows are kept together below, un-split, as historical record.
...
| F-40 | P2 (Versioning) has no rule requiring a DADMS ID be created when the major version number changes | P2 | open |
...
| F-37 | Four rules govern something with no external standard to check against, and were never labeled as a deliberate house convention: the umbrella-branch fast-forward exception (P1.2.3, process) has no external precedent; the poison-pill rebuild (now in `project/PROJECT_PROFILE.md`, process) never got the suggested explicit failure-mode checklist language; `@brief` third-person mood (C2, language) is not labeled as a house convention; local-constant camelBack (C1.12, language) is accepted as permanently advisory but never stated as such | P1.2.3, C2, C1.12, `project/PROJECT_PROFILE.md` | open |
...
Parked because the section that should hold them does not exist yet. Check this table when writing the named section. Grouped by domain.
...
| ID | Note | Check when writing |
|---|---|---|
| FW-06 | P1.6.2's process for adding to `test_data/` is still undocumented — approval step, size limits, manifest (approval step is process; size limits/manifest specifics belong with testing, a language-scoped topic). | § P1.6 / § 12 Testing |
...
| R-09 | How should a rule get structured when more than one mechanism could satisfy it (e.g., manual review vs. an automated pipeline), given different teams across the org have different tooling available? | Parent section states the requirement; each mechanism gets its own numbered subsection under it (e.g., P1.8 MR verification / P1.8.1 Manual verification sequence). No inline commentary that alternatives exist or "might change" — the structure itself leaves room for a sibling subsection (e.g., a future P1.8.2) to be added later via the normal "Adding a rule" process, without forward-referencing anything unwritten. Applies repo-wide, not just to CI/manual review. |
| R-10 | Should rule text ever address the reader directly, narrate the document's own editing history, or justify why something is being stated? | No. This is a rules/process/standards/informational document, not a conversation. RATIONALE explains why a rule exists, not why it's being written down — no "this document previously called it X," "reviewers should read it as...," "is itself a goal worth stating explicitly," etc. Applies everywhere, not only to already-reviewed files. |
